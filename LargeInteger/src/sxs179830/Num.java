...
public class Num  implements Comparable<Num> {
    ...
    public Num(String s) {
        if(s != null && s.length() > 0) {
            ...
            this.size = (s.length() / logVal) + 1;
            int i = 0;
            this.arr = new long[size];
            while (s != null && !s.equals("")) {
                Long l = Long.valueOf(s.substring(s.length() >= logVal ? s.length() - logVal : 0, s.length()));
                s = s.length() >= logVal ? s.substring(0, s.length() - logVal) : null;
                arr[i] = l;
                i++;
            }
        }
    ...
    public static Num product(Num a, Num b) {
        ...
        for(int i = 0; i< b.arr.length; i++) {
            ...
            Num nextToAdd = new Num(rightShiftBy(res, i), a.base, isNeg);
            mulResult = add(mulResult, nextToAdd);
        }
        mulResult.isNegative = isNeg;
        //assumption : operation can be only performed with same base number
        mulResult.base = a.base;
        return mulResult;
    }
...
//    	System.out.println("times" + times);
    ...
    public static void main(String[] args) {
        ...
        Num num1 = new Num("131567855987576546586768765465453545668769698676875867868");
        Num num2 = new Num("98676785765645565675876976744354565798696564345868978798");
